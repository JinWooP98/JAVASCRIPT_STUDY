...
const findMaxValue = traders
  .map((user) => user.value)
  .forEach((value) => {
    if (maxValue < +value) maxValue = +value;
  });

const maxValuetrader = traders
  .filter((user) => user.value === maxValue)
  .map((user) => ({
    name: user.trader.name,
    place: user.trader.city,
    value: user.value,
  }));
...
*/
let cityes2 = [];
const tradersInCity = traders
  .map((userAddr) => userAddr.trader.city)
  .forEach((city) => {
    if (!cityes2.includes(city)) cityes2.push(city);
  });
